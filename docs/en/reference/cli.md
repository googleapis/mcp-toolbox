...
|              | `--response-encoding`      | Specify the encoding for tool responses sent to clients. Allowed: 'json' (default) or 'gcf' ([Graph Compact Format](https://gcformat.com)). GCF is used only when it is smaller than JSON and a lossless round-trip.                    | `json`      |
