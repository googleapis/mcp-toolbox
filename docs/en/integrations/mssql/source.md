...
| user      |  string  |    true\*    | Name of the SQL Server user to connect as (e.g. "my-user"). Not required when `azureAuth` is set.                                                                                                                                                                          |
...
| clientId                   |  string  |    false     | Client ID of the identity. Set it where a host maps to more than one identity, so the credential is not left to be guessed.                         |
