...
type ConfigParser struct {
	...
	// placeholder. The substitution is final — nothing re-reads the environment
	// afterwards, so any source depending on one of these fails when it
	// connects. Callers report the list rather than let a placeholder stand in
	// for real config silently.
