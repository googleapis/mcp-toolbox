...
func TestParseEnv(t *testing.T) {
	tcs := []struct {
		...
		},
		{
			...
			want:      "",
			err:       true,
			errString: `environment variable not found: "FOO" (set but empty, line 1, column 1)`,
		},
		{
			desc:      "empty and unset required vars reported together",
			env:       map[string]string{"USER_NAME": ""},
			in:        "host: ${HOST}, user: ${USER_NAME}",
			want:      "host: , user: ",
			err:       true,
			errString: "environment variables not found:\n  - \"HOST\" (line 1, column 7)\n  - \"USER_NAME\" (set but empty, line 1, column 22)",
		},
		{
			desc:    "without default with empty env, lenient",
			env:     map[string]string{"FOO": ""},
			in:      "${FOO}",
			want:    "FOO",
			lenient: true,
		...
		{
			desc:         "empty optional with empty env",
			env:          map[string]string{"FOO": ""},
			in:           "${FOO:}",
			want:         "",
		...
		{
			desc:         "with default with empty env",
			env:          map[string]string{"FOO": ""},
			in:           "${FOO:bar}",
			want:         "",
			wantOptional: []string{"FOO"},
