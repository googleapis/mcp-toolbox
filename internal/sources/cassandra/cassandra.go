...
func (c Config) Initialize(ctx context.Context, tracer trace.Tracer, deferConnect bool) (sources.Source, error) {
	s := &Source{
		Config: c,
		conn:   sources.NewConnectOnce[*gocql.Session](ctx, c.Name, SourceType, c, tracer),
	}
	if deferConnect {
		return s, nil
	}
...
func (s *Source) session(ctx context.Context) (*gocql.Session, error) {
	return s.conn.DoWithConfig(ctx, func(ctx context.Context, sc sources.SourceConfig) (*gocql.Session, error) {
		r := sc.(Config)
...
// CassandraSession reports the session once connected.
