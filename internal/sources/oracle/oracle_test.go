...
func TestRunSQLExecutesDML(t *testing.T) {
	...
	src := &Source{
		Config: cfg,
		conn:   sources.NewConnectOnce[*sql.DB](context.Background(), cfg.Name, SourceType, cfg, noop.NewTracerProvider().Tracer("test")),
	}

	// Seed the lazy connection with the mock handle so RunSQL does not dial Oracle.
	if _, err := src.conn.DoWithConfig(context.Background(), func(context.Context, sources.SourceConfig) (*sql.DB, error) {
		return db, nil
	}); err != nil {
