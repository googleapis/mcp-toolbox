...
func (r Config) Initialize(ctx context.Context, tracer trace.Tracer, deferConnect bool) (sources.Source, error) {
	s := &Source{
		Config: r,
		conn:   sources.NewConnectOnce[neo4j.Driver](ctx, r.Name, SourceType, r, tracer),
	}
	if deferConnect {
		return s, nil
	}
...
func (s *Source) driver(ctx context.Context) (neo4j.Driver, error) {
	return s.conn.DoWithConfig(ctx, func(ctx context.Context, sc sources.SourceConfig) (neo4j.Driver, error) {
		r := sc.(Config)
		driver, err := initArcadeDBDriver(ctx, r.Uri, r.User, r.Password)
		if err != nil {
