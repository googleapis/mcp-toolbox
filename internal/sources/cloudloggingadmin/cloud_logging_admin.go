...
func (r Config) Initialize(ctx context.Context, tracer trace.Tracer, deferConnect bool) (sources.Source, error) {
	...
	s := &Source{
		Config: r,
		tracer: tracer,
		conn:   sources.NewConnectOnce[*clientSet](ctx, r.Name, SourceType, r, tracer),
	}
	if deferConnect {
		return s, nil
	}
...
func (s *Source) clients(ctx context.Context) (*clientSet, error) {
	return s.conn.DoWithConfig(ctx, func(ctx context.Context, sc sources.SourceConfig) (*clientSet, error) {
		...
		if r.UseClientOAuth {
			// use client OAuth
			baseClientCreator, err := newLogAdminClientCreator(ctx, s.tracer, r.Project, r.Name)
			if err != nil {
		...
		client, tokenSource, err := initLogAdminConnection(ctx, r.Project, r.ImpersonateServiceAccount)
		if err != nil {
...
// LogAdminClient reports the ADC-backed client once connected.
...
// LogAdminTokenSource reports the ADC token source if one has been made.
...
// LogAdminClientCreator reports the per-token client factory once connected.
