...
func (r Config) Initialize(ctx context.Context, tracer trace.Tracer, deferConnect bool) (sources.Source, error) {
	...
	s := &Source{
		Config: r,
		conn:   sources.NewConnectOnce[*sql.DB](ctx, r.Name, SourceType, r, tracer),
	}
	if deferConnect {
		return s, nil
	}
...
func (s *Source) pool(ctx context.Context) (*sql.DB, error) {
	return s.conn.DoWithConfig(ctx, func(ctx context.Context, sc sources.SourceConfig) (*sql.DB, error) {
		r := sc.(Config)
		db, err := initCloudSQLMssqlConnection(ctx, r.Name, r.Project, r.Region, r.Instance, r.IPType.String(), r.User, r.Password, r.Database)
		if err != nil {
...
func initCloudSQLMssqlConnection(ctx context.Context, name, project, region, instance, ipType, user, pass, dbname string) (*sql.DB, error) {
