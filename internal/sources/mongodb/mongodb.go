...
func (r Config) Initialize(ctx context.Context, tracer trace.Tracer, deferConnect bool) (sources.Source, error) {
	s := &Source{
		Config: r,
		conn:   sources.NewConnectOnce[*mongo.Client](ctx, r.Name, SourceType, r, tracer),
	}
	if deferConnect {
		return s, nil
	}
...
func (s *Source) client(ctx context.Context) (*mongo.Client, error) {
	return s.conn.DoWithConfig(ctx, func(ctx context.Context, sc sources.SourceConfig) (*mongo.Client, error) {
		r := sc.(Config)
		client, err := initMongoDBClient(ctx, r.Uri)
		if err != nil {
