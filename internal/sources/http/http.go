...
func (r Config) Initialize(ctx context.Context, tracer trace.Tracer, deferConnect bool) (sources.Source, error) {
	...
	s := &Source{
		Config: r,
		guard:  guard,
		conn:   sources.NewConnectOnce[*httpClient](ctx, r.Name, SourceType, r, tracer),
	}
	if deferConnect {
		return s, nil
	}
...
func (s *Source) client(ctx context.Context) (*httpClient, error) {
	return s.conn.DoWithConfig(ctx, func(ctx context.Context, sc sources.SourceConfig) (*httpClient, error) {
		r := sc.(Config)
		// Neither the timeout nor the base URL needs a network to check, but
		// both are only known to be final here, so this is where they are
		// rejected.
		...
		client, err := createHTTPClient(timeout, tr, s.guard, nil)
		if err != nil {
