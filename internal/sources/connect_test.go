...
func doOnce(once *sources.ConnectOnce[*handle], ctx context.Context, connect func(context.Context) (*handle, error)) (*handle, error) {
	return once.DoWithConfig(ctx, func(ctx context.Context, _ sources.SourceConfig) (*handle, error) {
		return connect(ctx)
	})
}
...
func newConnectOnce(ctx context.Context, opts ...sources.Option) *sources.ConnectOnce[*handle] {
	return sources.NewConnectOnce[*handle](ctx, "my-source", "mock", nil, noop.NewTracerProvider().Tracer("test"), opts...)
}
