...
func TestGenerateResourceManifest(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got := generateResourceManifest(tc.resName, tc.description, tc.uri, tc.mimeType)
			if diff := cmp.Diff(tc.want, got); diff != "" {
...
func TestGenerateResourceTemplateManifest(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got := generateResourceTemplateManifest(tc.tmplName, tc.description, tc.uriTemplate, tc.mimeType)
			if diff := cmp.Diff(tc.want, got); diff != "" {
...
func TestGenerateListResourcesResult(t *testing.T) {
	res1 := testutils.NewMockResource("res1", "file://res1", "Title 1", "Desc 1", "text/plain", nil, nil)
