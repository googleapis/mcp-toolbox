...
package lookerconversationalanalyticschat
...
const resourceType string = "looker-conversational-analytics-chat"
...
func (cfg Config) Initialize(ctx context.Context) (tools.Tool, error) {
	...
	userMessageParameter := parameters.NewStringParameter("user_message", "The text content of the message to send.", parameters.WithStringDefault(""))
	allParameters := parameters.Parameters{conversationIdParameter, userMessageParameter}
...
func (t Tool) Invoke(ctx context.Context, s sources.Source, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	var conversationId, userMessage string
	if v, ok := mapParams["conversation_id"].(string); ok {
		conversationId = v
	}
	if v, ok := mapParams["user_message"].(string); ok {
		userMessage = v
	}
	...
	if userMessage == "" {
		return nil, util.NewClientServerError(fmt.Sprintf("%s operation: user_message must be specified", t.Cfg.Type), http.StatusBadRequest, nil)
	}

	body := v4.ConversationalAnalyticsChatRequest{
		ConversationId: conversationId,
		UserMessage:    userMessage,
	}

	resp, err := sdk.ConversationalAnalyticsChat(body, source.LookerApiSettings())
	if err != nil {
