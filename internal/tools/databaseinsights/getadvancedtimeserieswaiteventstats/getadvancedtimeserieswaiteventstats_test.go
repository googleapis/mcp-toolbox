...
package getadvancedtimeserieswaiteventstats_test
...
import (
	...
	getstats "github.com/googleapis/mcp-toolbox/internal/tools/databaseinsights/getadvancedtimeserieswaiteventstats"
...
func TestParseFromYaml(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
            ...
            description: fetches wait event time-series history
            `,
			want: server.ToolConfigs{
				"get_wait_event_time_series": getstats.Config{
...
type mockSource struct {
	sources.Source
	t           *testing.T
	expectedReq *databaseinsights.FetchWaitEventTimeSeriesRequest
	resp        *databaseinsights.FetchWaitEventTimeSeriesResponse
	err         error
}

func (m *mockSource) FetchWaitEventTimeSeries(ctx context.Context, req *databaseinsights.FetchWaitEventTimeSeriesRequest) (*databaseinsights.FetchWaitEventTimeSeriesResponse, error) {
...
func TestInvoke(t *testing.T) {
	...
	mockSrc := &mockSource{
		...
		resp: &databaseinsights.FetchWaitEventTimeSeriesResponse{
			TimeSeries: []databaseinsights.TimeSeries{
				{
					GroupbyFieldValues: []string{"Lock"},
				},
			},
		},
	}
	...
	if diff := cmp.Diff(mockSrc.resp, got, cmpopts.IgnoreUnexported(databaseinsights.FetchWaitEventTimeSeriesResponse{})); diff != "" {
