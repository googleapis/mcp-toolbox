...
package getadvancedtimeseriesquerystats_test
...
import (
	...
	getstats "github.com/googleapis/mcp-toolbox/internal/tools/databaseinsights/getadvancedtimeseriesquerystats"
...
func TestParseFromYaml(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
            ...
            description: fetches query time-series history
            `,
			want: server.ToolConfigs{
				"get_query_time_series": getstats.Config{
...
type mockSource struct {
	sources.Source
	t           *testing.T
	expectedReq *databaseinsights.FetchQueryTimeSeriesRequest
	resp        *databaseinsights.FetchQueryTimeSeriesResponse
	err         error
}

func (m *mockSource) FetchQueryTimeSeries(ctx context.Context, req *databaseinsights.FetchQueryTimeSeriesRequest) (*databaseinsights.FetchQueryTimeSeriesResponse, error) {
...
func TestInvoke(t *testing.T) {
	...
	mockSrc := &mockSource{
		...
		resp: &databaseinsights.FetchQueryTimeSeriesResponse{
			TimeSeries: []databaseinsights.TimeSeries{
				{
					GroupbyFieldValues: []string{"query_123"},
				},
			},
		},
	}
	...
	if diff := cmp.Diff(mockSrc.resp, got, cmpopts.IgnoreUnexported(databaseinsights.FetchQueryTimeSeriesResponse{})); diff != "" {
