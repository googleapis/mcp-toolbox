...
package bigtabledeletelogicalview
...
const resourceType string = "bigtable-delete-logical-view"
...
type compatibleSource interface {
	DeleteLogicalView(context.Context, string, string) (any, error)
}
...
func (cfg Config) Initialize(context.Context) (tools.Tool, error) {
	if cfg.Description == "" {
		cfg.Description = "Delete a Bigtable logical view."
	}
...
func (t Tool) Invoke(ctx context.Context, src sources.Source, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	res, err := source.DeleteLogicalView(ctx, paramsMap["instance_id"].(string), paramsMap["logical_view_id"].(string))
	if err != nil {
