...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtabledeletelogicalview"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtabledeletelogicalview.Config{
...
func (m *mockSource) DeleteLogicalView(ctx context.Context, instanceID, logicalViewID string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtabledeletelogicalview.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		want := any(map[string]string{"status": "logical view deleted successfully"})
