...
package bigtablegetmaterializedview_test
...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtablegetmaterializedview"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtablegetmaterializedview.Config{
...
func (m *mockSource) GetMaterializedView(ctx context.Context, instanceID, materializedViewID string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
	if instanceID != "inst-1" || materializedViewID != "mview-1" {
		return nil, fmt.Errorf("unexpected arguments: instanceID=%q, materializedViewID=%q", instanceID, materializedViewID)
	}
	return &bigtable.MaterializedViewInfo{MaterializedViewID: materializedViewID, Query: "SELECT _key FROM t"}, nil
}
...
func TestInvoke(t *testing.T) {
	cfg := bigtablegetmaterializedview.Config{
