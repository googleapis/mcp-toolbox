...
package bigtabledeletematerializedview_test
...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtabledeletematerializedview"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtabledeletematerializedview.Config{
...
func (m *mockSource) DeleteMaterializedView(ctx context.Context, instanceID, materializedViewID string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtabledeletematerializedview.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		want := any(map[string]string{"status": "materialized view deleted successfully"})
