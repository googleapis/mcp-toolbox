...
package bigtablegetcluster_test
...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtablegetcluster"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtablegetcluster.Config{
...
func (m *mockSource) GetCluster(ctx context.Context, instanceID, clusterID string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
	return map[string]string{"cluster": "clus-1"}, nil
}

func TestInvoke(t *testing.T) {
	cfg := bigtablegetcluster.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		want := any(map[string]string{"cluster": "clus-1"})
