...
const resourceType string = "bigtable-delete-cluster"
...
type compatibleSource interface {
	DeleteCluster(context.Context, string, string) (any, error)
}
...
func (cfg Config) Initialize(context.Context) (tools.Tool, error) {
	if cfg.Description == "" {
		cfg.Description = "Delete a Bigtable cluster."
	}
...
func (t Tool) Invoke(ctx context.Context, src sources.Source, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	res, err := source.DeleteCluster(ctx, paramsMap["instance_id"].(string), paramsMap["cluster_id"].(string))
	if err != nil {
