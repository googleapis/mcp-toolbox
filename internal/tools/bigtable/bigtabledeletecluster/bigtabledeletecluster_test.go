...
package bigtabledeletecluster_test
...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtabledeletecluster"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtabledeletecluster.Config{
...
func (m *mockSource) DeleteCluster(ctx context.Context, instanceID, clusterID string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtabledeletecluster.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		want := any(map[string]string{"status": "cluster deleted successfully"})
