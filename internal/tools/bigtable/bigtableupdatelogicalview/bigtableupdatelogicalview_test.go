...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtableupdatelogicalview"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtableupdatelogicalview.Config{
...
func (m *mockSource) UpdateLogicalView(ctx context.Context, instanceID, logicalViewID, query string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtableupdatelogicalview.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		params := parameters.ParamValues{
			...
			{Name: "query", Value: "SELECT * FROM t2"},
		}
		got, err := tool.Invoke(context.Background(), src, params, "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		want := any(map[string]string{"status": "logical view updated successfully"})
