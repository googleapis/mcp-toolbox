...
package bigtablelistclusters_test
...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtablelistclusters"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtablelistclusters.Config{
...
func (m *mockSource) ListClusters(ctx context.Context, instanceID string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
	return []string{"clus-1"}, nil
}

func TestInvoke(t *testing.T) {
	cfg := bigtablelistclusters.Config{
