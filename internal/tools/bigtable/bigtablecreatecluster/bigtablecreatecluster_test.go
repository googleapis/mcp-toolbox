...
package bigtablecreatecluster_test
...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtablecreatecluster"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtablecreatecluster.Config{
...
func (m *mockSource) CreateCluster(ctx context.Context, instanceID, clusterID, zone string, numNodes int32) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtablecreatecluster.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		want := any(map[string]string{"status": "cluster created successfully"})
