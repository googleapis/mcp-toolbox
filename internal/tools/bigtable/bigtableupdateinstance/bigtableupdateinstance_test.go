...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtableupdateinstance"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtableupdateinstance.Config{
...
func (m *mockSource) UpdateInstance(ctx context.Context, instanceID, displayName string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtableupdateinstance.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		params := parameters.ParamValues{
			...
			{Name: "display_name", Value: "New Name"},
		}
		got, err := tool.Invoke(context.Background(), src, params, "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		want := any(map[string]string{"status": "instance updated successfully"})
