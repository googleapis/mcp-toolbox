...
package bigtabledeletetable_test
...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtabledeletetable"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtabledeletetable.Config{
...
func (m *mockSource) DeleteTable(ctx context.Context, tableID string) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtabledeletetable.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		want := any(map[string]string{"status": "table deleted successfully"})
