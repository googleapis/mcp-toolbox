...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtablegettable"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtablegettable.Config{
...
func (m *mockSource) GetTable(ctx context.Context, tableID string) (any, error) {
	...
	return map[string]string{"table": "t1"}, nil
}

func TestInvoke(t *testing.T) {
	cfg := bigtablegettable.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		want := any(map[string]string{"table": "t1"})
