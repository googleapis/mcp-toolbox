...
import (
	...
	"github.com/googleapis/mcp-toolbox/internal/tools/bigtable/bigtableupdatecluster"
...
func TestParseFromYamlBigtable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": bigtableupdatecluster.Config{
...
func (m *mockSource) UpdateCluster(ctx context.Context, instanceID, clusterID string, serveNodes int32) (any, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func TestInvoke(t *testing.T) {
	cfg := bigtableupdatecluster.Config{
	...
	t.Run("success", func(t *testing.T) {
		...
		params := parameters.ParamValues{
			...
			{Name: "serve_nodes", Value: 5},
		}
		got, err := tool.Invoke(context.Background(), src, params, "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		want := any(map[string]string{"status": "cluster updated successfully"})
