...
const resourceType string = "bigtable-update-cluster"
...
type compatibleSource interface {
	UpdateCluster(context.Context, string, string, int32) (any, error)
}
...
func (cfg Config) Initialize(context.Context) (tools.Tool, error) {
	if cfg.Description == "" {
		cfg.Description = "Update the number of nodes in a Bigtable cluster."
	}

	allParameters := parameters.Parameters{
		parameters.NewStringParameter("instance_id", "The ID of the instance"),
		parameters.NewStringParameter("cluster_id", "The ID of the cluster"),
		parameters.NewIntParameter("serve_nodes", "The new number of nodes to allocate"),
	}
